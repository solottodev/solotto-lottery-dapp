...
exports.getDrawingService = getDrawingService;
...
class DrawingService {
     ...
     */
    selectWinner(eligibleParticipants, seed, tierIndex) {
    ...
    async runDrawing(roundId) {
        ...
        const { getRPCService } = await Promise.resolve().then(() => __importStar(require('./rpc.service')));
        ...
        const eligibleByTier = await Promise.all([
            prisma_1.default.participant.findMany({
        ...
        if (winner1) {
            ...
            if (participant) {
                updatePromises.push(prisma_1.default.participant.update({
     ...
     */
    async verifyDrawing(roundId, seed) {
        ...
        const actualWinners = await prisma_1.default.participant.findMany({
        ...
        // Compare
        const verified = expectedWinner1 === actualWinner1 &&
...
// Singleton instance
let drawingServiceInstance = null;
